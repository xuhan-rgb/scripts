...
class CodexBridgeManagerTests(unittest.TestCase):
    ...
    def test_dashboard_renders_usage_periods_and_ttft_incrementally(self):
        ...
        self.assertIn("app.lastRefreshAt = Date.now()", manager.HTML)
        self.assertIn("setInterval(refreshDashboardAge, 1000)", manager.HTML)
        self.assertIn("`${seconds} 秒前刷新`", manager.HTML)
