...
''',
    ...
    def test_quota_card_can_show_a_draggable_desktop_overlay(self):
        ...
        self.assertIn("self.drag_timer.timeout.connect(self.track_drag)", source)
        self.assertIn("self.drag_timer.start()", source)
        self.assertIn("QCursor.pos()", source)
        self.assertIn("def finish_drag", source)
        self.assertNotIn("self.grabMouse()", source)
