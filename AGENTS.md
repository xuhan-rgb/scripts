...
- 修改模型用量脚本后运行 Shell 语法检查及 `test_codex_model_usage.py`；保持 token 用量与实际套餐额度的区别，测试使用合成日志，不提交真实会话原文。最近会话模式依赖 Python 3 标准库解析日志父子关系，按主会话及全部子代理聚合 ccusage 用量，问答预览取自主会话。
